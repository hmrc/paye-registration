...
class EmploymentISpec extends IntegrationSpecBase {
  ...
  "PAYE Registration API - Employment" should {
    ...
    "Return a 200 when the user gets employment" in new Setup {
      ...
      repository.insert(PAYERegistration(regID, transactionID, intID, Some("testAckRef"), timestamp, PAYEStatus.draft, None, None, Seq.empty, None, Some(validEmployment), Seq.empty))
