...
@Singleton
class HIPConnector @Inject()(val http: HttpClient, appConfig: AppConfig, val auditService: AuditService)
  extends BaseConnector with BaseHttpReads with HttpErrorFunctions with Logging {
  ...
  private def payePOST[I, O](url: String, body: I)(implicit wts: Writes[I], rds: HttpReads[O], hc: HeaderCarrier, ec: ExecutionContext) =
    http.POST[I, O](url, body, createHIPHeaders)(wts = wts, rds = rds, hc = hc, ec = ec)

  private def createHIPHeaders: Seq[(String, String)] = Seq(
    "Authorization"         -> appConfig.hipUrlHeaderAuthorization,
    "Environment"           -> appConfig.hipUrlHeaderEnvironment,
    "correlationid"         -> UUID.randomUUID().toString,
    "X-Originating-System"  -> appConfig.hipUrlHeaderOriginatingSystem,
    "X-Receipt-Date"        -> SystemDate.getSystemDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'")),
    "X-Transmitting-System" -> appConfig.hipUrlHeaderTransmittingSystem
  )
}
